...
public class JpegProcessor : IJpegProcessor
{
        ...
        for (var y = 0; y < matrix.Height; y += DCTSize)
        {
            for (var x = 0; x < matrix.Width; x += DCTSize)
            {
                foreach (var selector in new Func<Pixel, double>[] { p => p.Y, p => p.Cb, p => p.Cr })
                {
                    ...
                    var channelFreqs = DCTDefault.DCT2D(subMatrix);
                    var quantizedFreqs = QuantizeDefault(channelFreqs, quality);
            ...
            for (var y = 0; y < image.Height; y += DCTSize)
            {
                for (var x = 0; x < image.Width; x += DCTSize)
                {
                    ...
                    foreach (var channel in new[] { _y, cb, cr })
                    {
                        ...
                        DCTDefault.IDCT2D(channelFreqs, channel);
    ...
    private static byte[,] QuantizeDefault(double[,] channelFreqs, int quality)
    {
